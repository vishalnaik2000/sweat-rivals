...
src/
  ...
  lib/
    ...
    streak.ts           computeStreak(metric, byDay) — consecutive-day / clean-day streaks
...
- **Dashboard/Stats** fetches the user's `entries` for a fixed 90-day window once (range buttons just re-slice client-side) and, per subscribed metric, shows an aggregate (`sum`/`average`/`count` per the metric's `aggregation`) plus a mini bar chart (Recharts). Missing days render as faint bars; lower-is-better metrics use the `--warn` color. Chart colors are CSS theme vars so they follow dark/light.
- **Streaks** (`src/lib/streak.ts`, shown as 🔥N on Today + each Stats card). Good metrics: consecutive days with a positive log, with a one-day grace for an un-logged today. Avoid metrics (`direction:'lower'`): clean days since the last slip (a logged value >0), counted only once the user has ≥1 entry. Both pull a 90-day history; Today keeps its `hist` map in sync on each save so the count updates live.
